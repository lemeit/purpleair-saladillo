...
```
Worker "purpleair-saladillo-api" — scheduled() (Cron Trigger Cloudflare, cada 15 min)
    ↓ (API PurpleAir → binding env.DB, directo, sin HTTP intermedio)
...
**Migración desde GitHub Actions (agosto 2026)**: hasta agosto de 2026 la ingesta corría como `ingest_purpleair.py` disparado por un `cron:` de GitHub Actions cada 15 min. Ese cron empezó a saltear corridas (el último dato quedaba viejo por más de una hora sin ningún error ni corrida fallida visible) — la causa más probable, según la propia documentación de GitHub, es que los scheduled events "pueden demorarse durante períodos de alta carga", con los minutos redondos (:00/:15/:30/:45) como el momento de mayor congestión — justo donde caía este cron. La ingesta se migró al mismo Worker que ya servía la API de lectura (`worker/src/index.js`, función `scheduled()`), disparado por un **Cron Trigger de Cloudflare** (`worker/wrangler.toml`, `[triggers] crons`) en los minutos :03/:18/:33/:48 — así ni la ejecución ni la escritura en D1 dependen más de GitHub, solo el código fuente sigue viviendo ahí. `ingest_purpleair.py` y su workflow (`.github/workflows/purpleair-ingest.yml`) se mantienen en el repo, pero el workflow quedó **solo con `workflow_dispatch`** (disparo manual, sin `schedule:`) — sirve para un backfill puntual a mano o como respaldo si el Worker estuviera caído, ya no como el camino principal.

## Ingesta

| Camino | Descripción |
|--------|-------------|
| `worker/src/index.js` → `scheduled()` | **Camino principal.** Cron Trigger de Cloudflare (cada 15 min, ver Arquitectura arriba). Consulta la API de PurpleAir (`api.purpleair.com/v1/sensors`), convierte temperatura de Fahrenheit a Celsius, y escribe metadata + lectura en D1 vía el binding `env.DB` (sin API HTTP intermedia). También expone `POST /api/ingest-ahora` (protegido con secret `X-Ingest-Key`) para disparar una corrida manual sin esperar al cron. |
| `ingest_purpleair.py` | Respaldo/histórico. Misma lógica, en Python, escribiendo a D1 vía su API HTTP. Se corre a mano (local o `workflow_dispatch` en GitHub Actions) para backfill o si el Worker no está disponible. |
...
INGEST_KEY          # clave propia para disparar POST /api/ingest-ahora a mano
```

**GitHub Actions** (`.github/workflows/purpleair-ingest.yml`, solo usado si se dispara a mano):
